...
## Detecta tap, doble tap, long press, swipe, pinch y drag.
...
## Obtener nombre legible de la dirección del swipe.
...
## Retorna la cantidad de puntos de toque activos.
func get_touch_count() -> int:
	return touch_points.size()

## Retorna la posición del primer punto de toque.
...
## Procesa eventos de toque (presionar y soltar).
...
## Procesa eventos de arrastre en pantalla.
func _handle_screen_drag(event: InputEventScreenDrag) -> void:
	...
	if touch_points.size() == 2:
		...
		if initial_pinch_distance > 0:
			var delta: float = current_distance - initial_pinch_distance
			pinch.emit(delta)
			initial_pinch_distance = current_distance

## Maneja el inicio de un toque.
func _on_touch_down(position: Vector2) -> void:
	is_pressing = true
...
func _on_touch_up(position: Vector2) -> void:
	is_pressing = false
	var press_duration: float = (Time.get_ticks_msec() / 1000.0) - press_start_time
	var drag_distance: float = position.distance_to(press_start_position)

	# Detectar swipe
	if drag_distance > SWIPE_THRESHOLD:
		var direction: Vector2 = (position - press_start_position).normalized()
...
## Maneja el arrastre continuo de un toque.
func _on_touch_drag(position: Vector2, relative: Vector2) -> void:
	var drag_distance: float = position.distance_to(press_start_position)
