...
static func get_device_type() -> DeviceType:
	...
	var diagonal = screen_size.length()
	var dpi = DisplayServer.screen_get_dpi()
	
	# Estimar diagonal en pulgadas
	var diagonal_inches = diagonal / dpi if dpi > 0 else diagonal / 160.0
...
# Obtener tamaño de botón adaptado
...
static func get_safe_area_rect() -> Rect2i:
	...
	var safe_rect = Rect2i(Vector2i.ZERO, screen_size)
...
# Escalar nodo según tamaño de pantalla
static func scale_node_to_screen(node: Node2D, base_resolution: Vector2 = Vector2(1080, 1920)):
	var screen_size = get_viewport_size()
